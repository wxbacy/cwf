...
function(i) {
    ...
    layui.data.content = function(i) {
        ...
        return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/img\[([^\s]+?)\]/g,
        function(i) {
        ...
        }).replace(a(), "<$1 $2>").replace(a("/"), "</$1>").replace(/\[([^\s\[\]]{1,7})\]/g,
        function(i) {
    ...
    X = function() {
        var i = ["[):]", "[:D]", "[;)]", "[:-o]", "[:p]", "[(H)]", "[:@]", "[:s]", "[:$]", "[:(]", "[:'(]", "[:|]", "[(a)]", "[8o|]", "[8-|]", "[+o(]", "[<o)]", "[|-)]", "[*-)]", "[:-#]", "[:-*]", "[^o)]", "[8-)]", "[(|)]", "[(u)]", "[(S)]", "[(*)]", "[(#)]", "[(R)]", "[({)]", "[(})]", "[(k)]", "[(F)]", "[(W)]", "[(D)]"],
        a = {};
        return layui.each(i,
        function(i, e) {
        	var facei = i + 1;
            a[e] = layui.cache.dir + "images/easemob_face/ee_" + facei + ".png"
        }),
        a
    } (),
    ii = layui.stope,
    ...
    ti = {
        ...
        setSendHide: function(i, a) { (a || ti.setSend.box).hide().removeClass(ei)
        },
        face: function(i, a) {
            var n = "",
            l = _();
    ...
    i("layim", new c)
}).addcss("modules/layim/layim.css", "skinlayimcss");