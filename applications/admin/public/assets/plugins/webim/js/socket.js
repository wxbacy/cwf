    ...
    var im = {
        ...
        initListener: function (user,pwd) { //初始化监听
            ...
            conn.listen({
                ...
                onError: function ( message ) {
                	var error = message.data.data;
                	var errorObj = eval("("+error+")");//转换为json对象
                	layer.msg("环信："+errorObj.error_description, {offset:'20px', time:3000});
                },          //失败回调
        ...
        defineMessage: function (message,msgType) {
            ...
            if (message.delay) {//离线消息获取不到本地cachedata用户名称需要从服务器获取
                ...
                var data = {
        		    username: message.ext.username //消息来源用户名
    			    ,avatar: message.ext.headImageUrl //消息来源用户头像
    			    ,id: message.ext.userid //消息的来源ID（如果是私聊，则是用户id，如果是群聊，则是群组id）
    			    ,type: type //聊天窗口来源类型，从发送消息传递的to里面获取
    			    ,content: msg //消息内容
    			    ,cid: 0 //消息id，可不传。除非你要对消息进行一些操作（如撤回）
    			    ,mine: false //是否我发送的消息，如果为true，则会显示在右方
    			    ,fromid: message.ext.userid //消息的发送者id（比如群组中的某个消息发送者），可用于自动解决浏览器多窗口时的一些问题
        ...
        sendMsg: function (data) {  //根据layim提供的data数据，进行解析
            ...
            var imgContentRegex = /img\[([^\s\[\]]+?)\]/g;
            var fileContentRegex = /file\(([^\s\[\]]+?)\)/g;
            if (imgContentRegex.test(content)) {
            	var msg = new WebIM.message('img', id);
            	var url = content.replace(imgContentRegex, function(img){  //转义图片
                    var tmpurl = content.replace(/^img/g, '');
                    var tmpurl = tmpurl.replace('[', '');
                    var tmpurl = tmpurl.replace(']', '');
                    return tmpurl;
                });
            	$.ajax({
            ...
            } else if (fileContentRegex.test(content)) {
            	...
            	msg.body.ext.userid = window.user.user_id;
                msg.body.ext.sex = window.user.gender;
                msg.body.ext.username = window.user.nickname;
                msg.body.ext.headImageUrl = window.user.avatar;
